...
def load_data(path: str) -> pd.DataFrame:
    ...
    # Ensure expected columns exist (from your pipeline)
    needed = [
        "Acres",
        "County, State",
    ...
    # Commissions (as per your description)
...
# Handle if user selects single date
...
# Markup multiple range
mm_min = float(np.nanmin(df["markup_multiple"]))
mm_max = float(np.nanmax(df["markup_multiple"]))
markup_range = st.sidebar.slider("Markup multiple range (Sale / Total Cost)", min_value=float(mm_min), max_value=float(mm_max),
...
def sweet_spot_table(data: pd.DataFrame, target_days: int = 30, n_bins: int = 8) -> pd.DataFrame:
    """
    Bin markup_multiple into quantiles and compute probability of selling within target_days.
    """
    d = data.dropna(subset=["markup_multiple", "days_to_sale"]).copy()
    if len(d) < 10:
        return pd.DataFrame()

    # quantile bins (handles skew)
    try:
    ...
    g = d.groupby("mm_bin", observed=True).agg(
        ...
        median_net_profit_$=("net_profit_$", "median"),
    ).reset_index()

    # Pretty labels
    g["mm_bin"] = g["mm_bin"].astype(str)
    g = g.sort_values("median_markup")
    # "sweet spot" ranking: prioritize high probability + decent net margin
...
# Compounding cycles estimate: 365 / median days (or 60 if you want conservative)
cycles_est = (365 / median_days) if (n and median_days and median_days > 0) else np.nan
...
with tab1:
        ...
        if t30.empty:
            ...
            st.dataframe(
                t30[["mm_bin", "n", "median_markup", "median_days", "p_sell_within_target",
                     "median_net_profit_pct", "median_net_profit_$", "score"]],
            ...
            if len(best30) == 1:
                st.success(
                    f"Best (≤30d) bin by score: **{best30['mm_bin'].iloc[0]}** | "
    ...
    with right:
        ...
        if t60.empty:
            ...
            if len(best60) == 1:
                st.success(
                    f"Best (≤60d) bin by score: **{best60['mm_bin'].iloc[0]}** | "
    ...
    if len(d) >= 10:
        # Smooth-ish probability using rolling window after sorting
        ...
        fig = px.line(d, x="markup_multiple", y="p_sell_smooth",
                      labels={"markup_multiple": "Markup multiple (Sale / Total Cost)",
...
with tab2:
    ...
    if len(d) >= 3:
        ...
        # Correlation hint (not causal; just directional)
